...
import Contoso from "../../assets/sofia.png";
    ...
    const makeApiRequestWithoutCosmosDB = async (question: string, conversationId?: string) => {
        ...
        try {
            ...
            if (!abortController.signal.aborted) {
                let errorMessage = "Ocorreu um erro. Por favor tente novamente. Se o problema persistir, por favor contate um administrador.";
    ...
    const makeApiRequestWithCosmosDB = async (question: string, conversationId?: string) => {
        ...
        try {
            ...
            if (!response?.ok) {
                ...
                var errorResponseMessage = responseJson.error === undefined ? "Por favor tente novamente. Se o problema persistir, contate um administrador." : responseJson.error;
                let errorChatMsg: ChatMessage = {
                    id: uuid(),
                    role: ERROR,
                    content: `Ocorreu um erro ao gerar a resposta. O histórico do chat não será salvo desta vez. ${errorResponseMessage}`,
                    date: new Date().toISOString()
                }
                        ...
                        {!messages || messages.length < 1 ? (
                            <Stack className={styles.chatEmptyState}>
                                <img
                                    ...
                                    aria-hidden="true"
                                />
                                <h1 className={styles.chatEmptyStateTitle}>Qual a sua dúvida?</h1>
                                <h2 className={styles.chatEmptyStateSubtitle}>Estou aqui para responder suas perguntas</h2>
                            </Stack>
                        ) : (
